...
def extract():
    ...
    symbols_csv = Variable.get("stock_symbols", default_var="NVDA,ORCL")
...
with DAG(
    dag_id='two_stock_dag',
    start_date=datetime(2024, 9, 21),
    catchup=False,
    tags=['ETL'],
    schedule='30 1 * * *',             # run daily at 01:30
    default_args={"retries": 1, "retry_delay": timedelta(minutes=5)},
) as dag:

    target_table = "USER_DB_PLATYPUS.RAW.TWO_STOCK_V2"
    ...
    trigger_forecast = TriggerDagRunOperator(
        ...
        trigger_dag_id="lab_forecast_dag",
